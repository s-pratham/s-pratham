...
class shop
{
	public:
		...
		void updateproduct(int p,int q,char* v)
		{
			int tempqty;
			tempqty=qty[p];
			qty[p]=tempqty-q;
...
{
	fstream myadd;
	myadd.open("D:/project/address.txt");
...
int main()
{
	...
	switch(mch)
	{
		...
		case 'L':{cout<<"\n\t\t\t\t::~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~::\n\t\t\t\t::\t\t\tLogin\t\t\t::";
				log:fstream omysign;
				omysign.open("D:/project/signup.txt");
				cout<<"\n\n\t\t\t\t\tEnter Email: ";
				cin>>id;
				cout<<"\n\n\t\t\t\t\tEnter Password: ";
				cin>>pass;
	...
	do
	{
		...
		switch(ch)
		{
			case 1:system ("CLS");
					...
					{
						otp:system("D:\\project\\otp.exe");
						cout<<"\nEnter the generated OTP to open Admin mode: ";
						cin>>otp;
						fstream ot;
						ot.open("D:/project/OTP.txt");
						ot>>motp;
						if(motp==otp)
						{
							system("D:\\project\\admin.exe");
						}
						else
						{
							cout<<"You entered wrong otp...";
							goto otp;
						}
					}
					break;
			...
			case 3:do
					{
						...
						cout << left << setw(20) << "1)Mobile" << setw(20) << "2)TV" << "\n\n";
						cout << left << setw(20) << "3)Laptop" << setw(20) << " " << "\n\n";
									...
									do
									{
										...
										else if(a[op].qty[pch]>0)
										{
											...
											if(bch==1)
											{
												...
												if(op==1)
													a[op].updateproduct(pch,addqty,path1);
												else if(op==2)
													a[op].updateproduct(pch,addqty,path2);
												else if(op==3)
													a[op].updateproduct(pch,addqty,path3);
											}
												...
												do
												{
													...
													switch(bop)
													{
														case 1:cout<<"UPI";
																...
																sleep(5);
																cout<<"\n\nPayment Successful!\n";
																cout<<"Press any key to print the bill...";
																getch();
																system("D:\\project\\loading.exe");
																system ("CLS");
																if(pay>0)
																{
																	...
																	pay=0;
																	if(op==1)
																		a[op].updateproduct(pch,buyqty,path1);
																	else if(op==2)
																		a[op].updateproduct(pch,buyqty,path2);
																	else if(op==3)
																		a[op].updateproduct(pch,buyqty,path3);
																}
																else if(cpay>0)
																{
